...
app.formResponseProcessor = function (formId, requestPayload, responsePayload) {
	...
	if (formId == 'sessionCreate') {
		...
		window.location.assign(`${window.location.origin}/checks/all`)
	}
	...
	if (formId == 'accountEdit3') {
		...
		window.location.assign(`${window.location.origin}/account/deleted`)
	}
...
app.loadAccountEditPage = function () {
	...
	var phone =
		typeof app.config.sessionToken.phone == 'string' ? app.config.sessionToken.phone : false
	if (phone) {
		// Fetch the user data
		var queryStringObject = {
			phone: phone,
			...
			function (statusCode, responsePayload) {
				if (statusCode == 200) {
					...
					for (var i = 0; i < successCodeCheckboxes.length; i++) {
						if (
							responsePayload.successCodes.indexOf(parseInt(successCodeCheckboxes[i].value)) > -1
						) {
