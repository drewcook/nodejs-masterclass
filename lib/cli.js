/**
 * CLI-Related Tasks
 * This is a CLI tool that allows working with our API
 * It follows an event driven design for each command entered
 * Can also get OS stats and view our logs
 *
 */
...
cli.responders = {
	...
	// Use a child process to call the shell and use it to call 'ls' where all the directories live
	listLogs: str => {
		...
		ls.stdout.on('data', data => {
			...
			logFilenames.forEach(logFilename => {
				// Only log compressed logs, which are [logId]-[timestamp]
				if (
			...
			_logs.decompress(logFilename, (err, strData) => {
				if (!err && strData) {
					...
					arr.forEach(jsonString => {
						const logObject = helpers.parseJsonStringToObject(jsonString)
