...
const workers = {
	...
	validateCheckData: checkData => {
		...
		if (
			...
			debug('Error: Check data is invalid')
		}
	},
	/**
	...
	processCheckOutcome: (check, outcome) => {
		...
		_data.update('checks', check.id, checkData, err => {
			if (!err) {
				...
				debug('Error: Failed to save check data while processing check outcome')
			}
		})
	},
	/**
	...
	alertUserToStatusChange: check => {
		...
		helpers.sendTwilioSms(check.userPhone, message, err => {
			if (!err) {
				...
				debug('Error: Failed to send SMS to alert user who had a status change in their check')
			}
		})
	},
	/**
	...
	rotateLogs: () => {
		...
		_logs.list(false, (err, logs) => {
			if (!err && logs && logs.length > 0) {
				...
				debug(`Could not find any logs to rotated - ${err}`)
			}
		})
	},
}
